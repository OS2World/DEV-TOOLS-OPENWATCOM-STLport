...
#   EXE         - when set tells to build separate executable for each test case,
#                 note, that single stl_test.exe will be built either.
#   PREPROC     - when set enable generation of preprocessor output for each file.
#
...
# Generation of separate executables
!ifdef EXE
SEPARATE_EXE=1
!endif
!ifdef %EXE
...
# Generation of preprocessing output
!ifndef PREPROC
!ifdef %PREPROC
PREPROC=1
!endif
...
OBJECTS = &
    ...
    hmap1.obj hmmap1.obj hmset1.obj hset2.obj &
    ...
    vec1.obj vec2.obj vec3.obj vec4.obj &
    vec5.obj vec6.obj vec7.obj vec8.obj &
    $STAT_MODULE
...
CXX = $(CC)
LINK = wlink
CPPFLAGS = -D_STLP_NO_OWN_IOSTREAMS $(STL_INCL) -i. -bm -we $(DEBUG_FLAGS) $(CXXFLAGS)
...
# Build all.
#
all: $(TEST_EXE) .SYMBOLIC
    @%null

$(TEST_EXE) : bin bin/stl_test.obj $(OBJECTS)
    @%create bin/$^*.lnk
    @for %i in ($OBJECTS) do @%append bin/$^*.lnk file bin/%i
    @%append bin/$^*.lnk file bin/stl_test.obj
    $(LINK) $(OPT_LINK) system os2v2 name $^@ opt symf @bin/$^*.lnk


# Main object file
bin/stl_test.obj : stl_test.cpp .AUTODEPEND
    $(CXX) $(CPPFLAGS) $[@ -fo=$^@ -fr=bin
!   ifdef PREPROC
        $(CXX) $(CPPFLAGS) -pcl $[@ -fo=bin/$^&.i -fr=bin
!   endif
...
!   ifdef PREPROC
        $(CXX) $(CPPFLAGS) -pcl $[&.cpp -fo=bin/$^&.i -fr=bin
!   endif


# Build separate executable
.exe: bin
.cpp.exe: .AUTODEPEND
!   ifndef SEPARATE_EXE
!   ifdef PREPROC
...
!   endif
!   endif
    $(CXX) -DMAIN=1 $(CPPFLAGS) $[*.cpp -fo=bin/$^&.obi -fr=bin
!   ifdef SEPARATE_EXE
        $(LINK) $(OPT_LINK) debug dwarf system os2v2 name bin/$^. file bin/$^&.obi
!   else
        $(LINK) $(OPT_LINK) debug dwarf system os2v2 name $^@ file bin/$^&.obi
!   endif
