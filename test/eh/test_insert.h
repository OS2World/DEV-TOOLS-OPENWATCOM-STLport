...
{
    C::const_iterator first1 = original.begin();
    C::const_iterator first2 = result.begin();

    C::const_iterator* from_orig = new C::const_iterator[original.size()];
    C::const_iterator* last_from_orig = from_orig;

    // fbp : for hashed containers, the following is needed :
    while ( first2 != result.end() )
    {
        EH_STD::pair<C::const_iterator, C::const_iterator> p = EH_STD::mismatch( first1, original.end(), first2 );
...
template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, sequence_container_tag)
{
    C::iterator pos = c.begin();
    EH_STD::advance(pos, offset);
    c.insert(pos, first, last);
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, associative_container_tag)
{
    c.insert( first, last );
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, multiset_tag)
{
    c.insert( first, last );
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, multimap_tag)
{
    c.insert( first, last );
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, set_tag)
{
    c.insert( first, last );
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last, map_tag)
{
    c.insert( first, last );
}

template <class C, class Iter>
void do_insert_range(C &c, size_t offset, Iter first, Iter last)
{
    do_insert_range(c, offset, first, last, container_category(c));
}
...
struct test_insert_range
{
    ...
    void operator()( C& c ) const
    {
        ...
        do_insert_range(c, fPos, fFirst, fLast);
        // Prevent simulated failures during verification
        gTestController.CancelFailureCountdown();
        // Success. Check results.
...
template <class C, class Iter>
test_insert_range<C, Iter>
    insert_range_at_begin_tester( const C& orig, const Iter& first, const Iter& last)
{
...
template <class C, class Iter>
test_insert_range<C, Iter>
    insert_range_at_end_tester( const C& orig, const Iter& first, const Iter& last)
{
